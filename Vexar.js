...
async function safeEdit(ctx, content, keyboard) {
  try {
    if (ctx.update.callback_query.message.photo) {
      return await ctx.editMessageCaption(content, {
        parse_mode: "HTML",
        ...keyboard
      });
    } else {
      return await ctx.editMessageText(content, {
        parse_mode: "HTML",
        ...keyboard
      });
    }
...
<blockquote>©️Dyysz || #𝑽𝒆𝒙𝒂𝑟𝑿𝒙 – 𝒊𝒏𝒇𝒊𝒏𝒊𝒕𝒚</blockquote>`,
    Markup.inlineKeyboard([
      [ Markup.button.callback("⬅ Back", "BACK_TO_START") ]
    ])
  );
